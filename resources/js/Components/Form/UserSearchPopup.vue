<template>
    <Popup title="Nutzer suchen" ref="popup" style="--max-width: 500px">
        <div class="flex vertical gap-1 padding-1">
            <mui-input type="search" ref="searchInput" v-model="form.filter.search" label="Nutzer suchen" @update:modelValue="fetch"/>
            <div class="flex vertical gap-0-5">
                <button class="select-button" type="button" v-for="item in form.items" @click="done(item)">
                    ...
                    <span class="text username">{{ item.username }}</span>
                </button>
            </div>
        </div>
    </Popup>
...
<script setup>
    ...
    const fetch = async () => {
        ...
        try
        {
            let response = await axios.get(route('admin.search.users', {...form.data(), items: []}))
